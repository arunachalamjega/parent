...
public class DaoServicio extends AbstractDao<Servicio> implements IDaoServicio {
    ...
    public Servicio buscarServicioPorUrl(String url) {
        ...
        query.setParameter("url", url);
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "url="+url);
        List<Servicio> lista = ejecutarSelect(query);
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "query="+query);
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, lista.toString());
        if(lista != null && lista.size() > 0)
            return lista.get(0);
